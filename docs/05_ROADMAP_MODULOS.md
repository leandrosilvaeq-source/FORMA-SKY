...
passando). Dois blocos de validação manual do usuário já aprovados em 2026-08-26: correção do
bug de empresas inativas em Clientes (vínculo criado com empresa ativa, preservado após
desativação, visível e selecionável na edição) e composição de produto com acessório/embalagem
sem custo (composição não bloqueia, Ficha Técnica exibe custo não informado e subtotal não
calculável/parcial corretamente). Integração E2E do restante do módulo (**Pedidos ponta a ponta**:
criar, itens múltiplos, tipos, status, pagamento, prazo, entrega, persistência) segue **sem
validação manual**; hospedagem pública ainda inativa (Vercel configurada em código desde
`ecfc510`, mas publicação **deliberadamente adiada** até a conclusão desta roadmap — ver §9).
Piloto real e estabilização/release **não iniciados**. **Módulo 1 não está concluído.**

## Próximo marco

Fase 4 (Frontend) do Módulo 1 concluída; dois blocos de validação manual aprovados pelo usuário em
2026-08-26 (Clientes/Empresas — bug de empresas inativas; Produtos — composição com
acessório/embalagem sem custo e leitura correta na Ficha Técnica), avançando a Fase 5 (integração
E2E manual) para ~20%. Próximo passo: estender a validação manual ao fluxo completo de **Pedidos**
(criar, itens múltiplos, tipos, status, pagamento, prazo, entrega, persistência) antes de
