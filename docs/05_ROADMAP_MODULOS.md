...
| 1 | Clientes, Produtos e Pedidos | 4×100% (F1–F4) + 1×~20% (F5) + 2×0% (F6–F7) | ~60% | 🟡 Frontend completo; Fase 5 (E2E) em andamento — Clientes/Empresas e composição de Produtos validados manualmente em 2026-08-26; Pedidos ainda sem validação E2E |
...
> Cálculo exato do Módulo 1: 4 fases 100% + 1 fase ~20% + 2 fases 0% = 4,2/7 = 60%.
...
**Percentual macro: (100+100+100+100+20+0+0) / 7 = 420/7 = 60%.**
...
Clientes, Empresas, Produtos e Pedidos funciona localmente (1092/1092 testes automatizados
passando, lint e build sem erros). Dois blocos de validação manual do usuário já aprovados em
2026-08-26: correção do bug de empresas inativas em Clientes (vínculo criado com empresa ativa,
preservado após desativação, visível e selecionável na edição) e composição de produto com
acessório/embalagem sem custo (composição não bloqueia, Ficha Técnica exibe custo não informado e
subtotal não calculável/parcial corretamente). **Na mesma data, em rodada separada**, o fluxo de
gerenciamento de Pedidos (status, cancelamento, pagamento, filtros rápidos, correção de vínculos
desativados, histórico) foi **implementado e coberto por teste automatizado**, mas **ainda sem
nenhuma validação manual do usuário na tela** — checklist de validação entregue ao final desta
rodada. Integração E2E do módulo (**Pedidos ponta a ponta**: criar, itens múltiplos, tipos,
alterar status, registrar pagamento, prazo, entrega, persistência) segue **sem validação manual**
para nenhum desses fluxos, apesar de agora todos estarem implementados; hospedagem pública ainda
inativa (Vercel configurada em código desde `ecfc510`, mas publicação **deliberadamente adiada**
até a conclusão desta roadmap — ver §9). Piloto real e estabilização/release **não iniciados**.
**Módulo 1 não está concluído.**

## Próximo marco

Fase 4 (Frontend) do Módulo 1 concluída, agora incluindo o fluxo completo de gerenciamento de
Pedidos (status/cancelamento/pagamento/filtros/histórico, `OrderManagementPanel.tsx`) implementado
e testado nesta mesma data (2026-08-26). Dois blocos de validação manual aprovados pelo usuário
também em 2026-08-26 (Clientes/Empresas — bug de empresas inativas; Produtos — composição com
acessório/embalagem sem custo e leitura correta na Ficha Técnica) mantêm a Fase 5 (integração E2E
manual) em ~20% — o gerenciamento de Pedidos recém-implementado **ainda não foi validado
manualmente**. Próximo passo: validação manual do usuário do checklist entregue ao final desta
rodada (criar pedido, avançar status até Entregue, cancelar um pedido em QUOTE, registrar
pagamento parcial e integral, filtrar por status, confirmar vínculo de cliente/empresa/produto
desativado preservado na edição) antes de considerar a Fase 5 concluída e iniciar a Fase 6 (piloto
