...
| 1 | Clientes, Produtos e Pedidos | 4×100% (F1–F4) + 1×~55% (F5) + 2×0% (F6–F7) | ~65% | 🟡 Frontend completo; fluxo de Pedidos (status/pagamento/cancelamento/filtros) validado manualmente em 2026-08-26 — refinamento visual seguinte ainda aguarda validação |
...
> Cálculo exato do Módulo 1: 4 fases 100% + 1 fase ~55% + 2 fases 0% = 4,55/7 ≈ 65%.
...
**Percentual macro: (100+100+100+100+55+0+0) / 7 = 455/7 = 65%.**
...
Clientes, Empresas, Produtos e Pedidos funciona localmente (1130/1130 testes automatizados
passando, lint e build sem erros). Três blocos de validação manual do usuário aprovados em
2026-08-26: (1) correção do bug de empresas inativas em Clientes; (2) composição de produto com
acessório/embalagem sem custo; (3) **fluxo completo de Pedidos** — criar pedido, avançar status,
registrar pagamento, avançar até entrega, cancelar e usar os filtros rápidos por status,
validados manualmente pelo usuário na interface (na forma em que existia antes do refinamento
visual desta mesma data). **Na sequência, ainda em 2026-08-26**, essas mesmas telas de Pedidos
foram refinadas visualmente (action buttons de Tipo/Método de pagamento, campo de valor no padrão
bancário, `OrderForm.tsx` em 3 seções, status alterável direto da listagem) — payloads, regras e
testes automatizados preservados, mas **essa superfície nova ainda não foi validada manualmente
pelo usuário** — checklist específico entregue ao final desta rodada. Hospedagem pública ainda
...
(status/cancelamento/pagamento/filtros/histórico) e, na sequência da mesma data, um refinamento
visual dessas mesmas telas (`RegisterPaymentForm.tsx` com action buttons e campo monetário
bancário; `OrderForm.tsx` em 3 seções; status alterável direto da listagem via
`OrderStatusControl.tsx`). O fluxo funcional principal de Pedidos (criar, avançar status,
registrar pagamento, entregar, cancelar, filtrar) já foi **validado manualmente pelo usuário em
2026-08-26**, elevando a Fase 5 para ~55% — mas essa validação foi feita na interface **anterior**
ao refinamento visual. Próximo passo: validação manual do usuário especificamente do checklist
desta rodada (action buttons de pagamento, campo de valor formatando em R$, as 3 seções do
formulário de pedido na ordem pedida, e o novo controle de status na própria listagem) antes de
considerar a Fase 5 concluída e iniciar a Fase 6 (piloto real). Publicação no Vercel continua fora
de escopo até a conclusão da roadmap (ver §9).
