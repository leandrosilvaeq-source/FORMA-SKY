...
**Migration NÃO aplicada ao Supabase remoto nesta rodada** — só existe localmente, na branch
`feature/inventory-operations`. Aplicar exige autorização explícita separada.

**Testes realmente executados nesta rodada**: suíte completa do frontend (1167/1167, sem
regressão — nenhum arquivo TypeScript foi alterado), lint (0 erros), build (sem erros), `git diff
--check` (limpo). **O teste de integração SQL (`inventory_movements_test.sql`) não pôde ser
executado nesta sessão** — o ambiente não tem Docker/Postgres local, e a migration não foi aplicada
ao remoto (não aplicá-la foi uma restrição explícita desta rodada) — só foi revisado linha a linha
contra a migration. Execução real fica pendente para quando a migration for aplicada (local via
Docker, ou remoto com autorização).
...
validado em produção' exigido pela regra de §1"). A fórmula oficial (`# 1`) só concede peso de
