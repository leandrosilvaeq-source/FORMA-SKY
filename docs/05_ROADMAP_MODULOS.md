...
| 1 | Clientes, Produtos e Pedidos | 4×100% (F1–F4) + 1×~60% (F5) + 2×0% (F6–F7) | ~66% | 🟡 Frontend completo; fluxo de Pedidos (status/pagamento/cancelamento/filtros) e o refinamento visual seguinte já validados manualmente em 2026-08-26 — status financeiro na listagem (extensão mais recente) ainda aguarda validação |
...
> Cálculo exato do Módulo 1: 4 fases 100% + 1 fase ~60% + 2 fases 0% = 4,6/7 ≈ 66%.
...
**Percentual macro: (100+100+100+100+60+0+0) / 7 = 460/7 ≈ 66%.**
...
Clientes, Empresas, Produtos e Pedidos funciona localmente (1155/1155 testes automatizados
passando, lint e build sem erros). Quatro blocos de validação manual do usuário aprovados em
...
registrar pagamento, avançar até entrega, cancelar, filtrar por status); (4) **as seis alterações
visuais do commit `8ea17c0`** (action buttons de Tipo/Método de pagamento, campo de valor no
padrão bancário, `OrderForm.tsx` em 3 seções, status do pedido alterável direto da listagem) —
todas validadas manualmente pelo usuário na interface. **Na sequência, ainda em 2026-08-26**, uma
nova extensão foi implementada: status financeiro também alterável direto da listagem
(`OrderPaymentStatusControl.tsx`, reaproveitando o mesmo `RegisterPaymentForm.tsx`/
`registerPayment()`) — payloads, regras e testes automatizados preservados, mas **essa extensão
específica ainda não foi validada manualmente pelo usuário** — checklist entregue ao final desta
rodada. Hospedagem pública ainda inativa (Vercel configurada em código desde `ecfc510`, mas
publicação **deliberadamente adiada** até a conclusão desta roadmap — ver §9). Piloto real e
estabilização/release **não iniciados**. **Módulo 1 não está concluído.**
