...
const SECTION_TITLE_CLASSNAME =
  'border-brand-primary/20 text-brand-primary-dark border-b pb-2 font-heading text-base font-semibold'
...
export function ProductForm({
  ...
  const [productType, setProductType] = useState<ProductType>(
    initialValues?.productType ?? 'CATALOG',
  )
  ...
  const [allowsPersonalization, setAllowsPersonalization] = useState(
    initialValues?.allowsPersonalization ?? true,
  )

  const [isManualAdjustment, setIsManualAdjustment] = useState(
    initialValues != null &&
      (initialValues.manualWeightOverrideGrams !== null ||
        initialValues.manualTimeOverrideSeconds !== null),
  )
  const [manualWeightInput, setManualWeightInput] = useState(
    initialValues?.manualWeightOverrideGrams != null
      ? String(initialValues.manualWeightOverrideGrams)
      : '',
  )
  ...
  function removePlate(key: string) {
    ...
    setPlates((current) =>
      current.length <= 1 ? current : current.filter((plate) => plate.key !== key),
    )
  }

  function updatePlateTime(key: string, timeInput: string) {
    setPlates((current) =>
      current.map((plate) => (plate.key === key ? { ...plate, timeInput } : plate)),
    )
  }

  function updatePlateWeight(key: string, weightInput: string) {
    setPlates((current) =>
      current.map((plate) => (plate.key === key ? { ...plate, weightInput } : plate)),
    )
  }

  function enableManualAdjustment() {
    setManualWeightInput((current) => (current.trim() ? current : String(autoWeight)))
    setManualTimeInput((current) =>
      current.trim() ? current : formatSecondsToHHM_or_empty(autoTimeSeconds),
    )
  ...
  // ---------------------------------------------------------------------
  // Seção 3 — Acessórios e Embalagem: campos INLINE dentro deste mesmo
  // formulário (rodada corretiva 2026-08-30) — nunca mais abre uma segunda
  // janela ("Selecionar acessórios e embalagens" foi removido). Mesmo
  // padrão de interação já usado pelos plates (Seção 2): "Adicionar
  // acessório"/"Adicionar embalagem" inserem uma linha vazia na hora, sem
  // sair deste formulário — o salvamento real só acontece quando o
  // formulário inteiro é submetido, exatamente como antes. Reaproveita as
  // mesmas linhas/validações já usadas por ProductCompositionForm.tsx
  // (agora fatoradas em CompositionRowsField + lib/forms/productComposition)
  // — nenhuma regra de duplicidade/item inativo/quantidade fora do
  // intervalo foi reescrita, só deixou de precisar de um <form> próprio.
  const [accessoryRows, setAccessoryRows] = useState<CompositionRow[]>(() =>
    (initialValues?.accessories ?? []).map((item) => ({
      key: nextRowKey(),
      itemId: item.id,
  ...
  const [packagingRows, setPackagingRows] = useState<CompositionRow[]>(() =>
    (initialValues?.packaging ?? []).map((item) => ({
      key: nextRowKey(),
      itemId: item.id,
      quantity: String(item.quantity),
    })),
  )
  const [compositionRowErrors, setCompositionRowErrors] = useState<Record<string, string>>({})

  // Snapshot fixo (por row.key) das quantidades já fora de 1-20 no momento
  // em que o Produto foi carregado para edição — mesma razão de
  // ProductCompositionForm.tsx (nunca recalculado depois, ou a opção extra
  // do Select some no meio da interação).
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (isManualAdjustment) {
      const weightResult = parseNumberField(manualWeightInput, 'o peso efetivo', {
        required: true,
        min: 0,
      })
    ...
    const compositionRowErrorsResult = { ...accessoryResult.errors, ...packagingResult.errors }
    setCompositionRowErrors(compositionRowErrorsResult)

    if (
      Object.keys(errors).length > 0 ||
      Object.keys(platesResult.timeErrors).length > 0 ||
      Object.keys(platesResult.weightErrors).length > 0 ||
      Object.keys(compositionRowErrorsResult).length > 0
    ) {
    ...
    onSubmit({
      ...
      accessories: accessoryResult.items,
      packaging: packagingResult.items,
    })
  }
    ...
    <form className="flex w-full max-w-full flex-col gap-4" onSubmit={handleSubmit}>
      ...
      >
        ...
        <div className={SECTION_CARD_CLASSNAME}>
          ...
          {/* Nome + Tipo do produto lado a lado no desktop — largura ampliada
            do Dialog (ProductsPage.tsx) sobra espaço aqui, e junta 2 campos
            numa linha só reduz a altura total do formulário. */}
            ...
            {mode === 'create' ? (
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-muted-foreground text-sm">
                  {PRODUCT_TYPE_ITEMS.find((item) => item.value === productType)?.label ??
                    productType}
                </p>
              </div>
            )}
          ...
          <div className="flex flex-col gap-2">
            ...
            <p id={CATEGORY_HELP_ID} className="text-muted-foreground text-xs">
              Um produto pode ter mais de uma categoria — selecione todas as que se aplicam ou
            ...
            {fieldErrors.categories && (
              <p className="text-destructive text-sm">{fieldErrors.categories}</p>
            )}
          </div>

          {/* Descrição + Preço lado a lado no desktop (só na criação, onde
            Preço ainda é editável aqui — ver comentário abaixo); em edição
            Descrição fica sozinha, já que Preço vira só uma nota. */}
          <div className={cn('grid grid-cols-1 gap-4', mode === 'create' && 'md:grid-cols-2')}>
            <div className="flex flex-col gap-2">
            ...
            {mode === 'create' ? (
              <div className="flex flex-col gap-2">
                ...
                <Input
                  ...
                  aria-describedby={
                    priceError || fieldErrors.default_price ? PRICE_ERROR_ID : undefined
                  }
        ...
        <div className={SECTION_CARD_CLASSNAME}>
          ...
          <p className="text-muted-foreground text-xs">
            Filamentos e cores não fazem mais parte do cadastro do Produto — são escolhidos no
            Pedido, por unidade e por plate.
          </p>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center gap-1">
              <Button
                ...
                onClick={() =>
                  plates.length > 0 && requestRemovePlate(plates[plates.length - 1].key)
                }
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            {plates.map((plate, plateIndex) => (
              <div
                key={plate.key}
                className="border-input bg-background flex flex-col gap-3 rounded-lg border p-3"
              >
              ...
              <div className="flex flex-col gap-3">
                <div className="flex flex-wrap items-end gap-3">
                  <div className="flex flex-col gap-1">
                    ...
                    />
                    {manualWeightError && (
                      <p className="text-destructive text-sm">{manualWeightError}</p>
                    )}
                  </div>
                  <div className="flex flex-col gap-1">
        ...
        {/* ------------------------------------------------------------- */}
        {/* Seção 3 — Acessórios e Embalagem — campos INLINE, nunca uma    */}
        {/* segunda janela (rodada corretiva 2026-08-30); lado a lado no   */}
        {/* desktop quando há espaço (md:grid-cols-2), mesmo padrão de     */}
        {/* Adicionar/linha/Remover já usado pelos plates na Seção 2.      */}
        {/* ------------------------------------------------------------- */}
        <div className={SECTION_CARD_CLASSNAME}>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <CompositionRowsField
              ...
              rows={accessoryRows}
              disabled={isSubmitting}
              rowErrors={compositionRowErrors}
          ...
          {fieldErrors.composition && (
            <p className="text-destructive text-sm">{fieldErrors.composition}</p>
          )}
        </div>
      </div>
