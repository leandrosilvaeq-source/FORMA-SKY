// Handler + validadores da Edge Function `packaging` — sem nenhum efeito
// colateral de módulo (nenhum Deno.serve aqui). Espelha integralmente
// supabase/functions/accessories/handler.ts (Incremento 2) — `packaging`
// tem exatamente a mesma estrutura de `accessories` (confirmado por leitura
// da Migration 18), então este arquivo reproduz o mesmo contrato ponto a
// ponto. `index.ts` é só o entrypoint (import + Deno.serve(handleRequest));
// nenhuma lógica vive lá.
//
// Rotas:
//   POST   /packaging       -> RPC create_packaging
//   PATCH  /packaging/:id   -> RPC update_packaging (edição e/ou ativar/desativar)
//   DELETE /packaging/:id   -> RPC delete_packaging (exclusão protegida)
//
...
// frontend. INSERT/UPDATE diretos de `authenticated` em public.packaging
...
// (frontend/src/lib/api/packaging.ts, listPackaging) — GRANT SELECT a
// authenticated preservado, RLS "Active users can view packaging"
...
// localmente — mesmo critério de accessories/handler.ts; AppError já aceita
// status arbitrário no construtor, então isso não exige alterar nenhum
...
// chave `size` não é enviada no PATCH (ver update_packaging).
const PACKAGING_SIZES = ["PP", "P", "M", "G", "GG"] as const;
...
// ignorados.
const PACKAGING_KEYS = ["name", "size", "variant", "minimum_stock", "is_active"] as const;

// Extraído como função nomeada para poder ser exercitado por testes locais
// sem precisar de um listener HTTP real — ver handler.test.ts. Mesmo padrão
// de accessories/handler.ts.
export async function handleRequest(req: Request): Promise<Response> {
  ...
  try {
    ...
    const anchor = segments.indexOf("packaging");
    const route = anchor >= 0 ? segments.slice(anchor + 1) : segments;

    if (route.length === 0) {
      if (req.method === "POST") return await handleCreatePackaging(req);
      throw new AppError("validation", 405, `Método ${req.method} não permitido em /packaging.`);
    }

    if (route.length === 1) {
      if (req.method === "PATCH") return await handleUpdatePackaging(req, route[0]);
      if (req.method === "DELETE") return await handleDeletePackaging(req, route[0]);
      throw new AppError("validation", 405, `Método ${req.method} não permitido em /packaging/:id.`);
    }
...
// rota" já usado em accessories/handler.ts, products/index.ts e
// order-items/index.ts — duplicados deliberadamente aqui em vez de
// compartilhados com accessories, mesma decisão já tomada entre
// products/index.ts e order-items/index.ts para seus próprios validadores).
// ---------------------------------------------------------------------------
...
export function optionalSize(value: unknown, field: string): string | null {
  ...
  if (!(PACKAGING_SIZES as readonly string[]).includes(trimmed)) {
    throw new ValidationError(
      `Campo inválido: ${field} deve ser um de: ${PACKAGING_SIZES.join(", ")} (ou vazio/null para "Não se aplica").`,
    );
  }
  return trimmed;
}
...
// current_stock incluídos, já que nenhum dos três está em PACKAGING_KEYS).
...
// ---------------------------------------------------------------------------
// POST /packaging -> create_packaging(p_name, p_size, p_variant,
//   p_minimum_stock, p_is_active, p_changed_by)
// ---------------------------------------------------------------------------
export function validateCreatePackagingPayload(body: Record<string, unknown>): Record<string, unknown> {
  rejectUnknownKeys(body, PACKAGING_KEYS, "corpo da requisição");
...
async function handleCreatePackaging(req: Request): Promise<Response> {
  const operator = await resolveOperator(req);

  const rawBody = await req.text();
  rejectIdentityFields(rawBody);
  const body = parseJsonBody(rawBody);
  const params = validateCreatePackagingPayload(body);

  const admin = getAdminClient();
  const { data, error } = await admin.rpc("create_packaging", {
    ...params,
...
// ---------------------------------------------------------------------------
// PATCH /packaging/:id -> update_packaging(p_packaging_id, p_patch, p_changed_by)
//
...
// constraint — mesmo cuidado já usado em accessories/handler.ts.
// ---------------------------------------------------------------------------
export function buildPackagingPatch(body: Record<string, unknown>): Record<string, unknown> {
  rejectUnknownKeys(body, PACKAGING_KEYS, "corpo do PATCH");
...
async function handleUpdatePackaging(req: Request, packagingId: string): Promise<Response> {
  const operator = await resolveOperator(req);

  if (!isUuid(packagingId)) {
    throw new ValidationError("Identificador de embalagem inválido na rota.");
  }

  const rawBody = await req.text();
  rejectIdentityFields(rawBody);
  const body = parseJsonBody(rawBody);
  const patch = buildPackagingPatch(body);

  const admin = getAdminClient();
  const { data, error } = await admin.rpc("update_packaging", {
...
// ---------------------------------------------------------------------------
// DELETE /packaging/:id -> delete_packaging(p_packaging_id, p_changed_by)
//
// Sem corpo. Toda a regra de bloqueio (vínculo em product_packaging) vive
...
// por _shared/errors.ts via o marcador estável PACKAGING_IN_USE:, ver
// RAISE_EXCEPTION_PATTERNS). Nenhuma confirmação do frontend é confiada
// como regra de segurança — quem decide bloquear é exclusivamente a RPC.
// ---------------------------------------------------------------------------
async function handleDeletePackaging(req: Request, packagingId: string): Promise<Response> {
  const operator = await resolveOperator(req);
  ...
  const admin = getAdminClient();
  const { error } = await admin.rpc("delete_packaging", {
    p_packaging_id: packagingId,
    p_changed_by: operator.userId,
  });
