...
-- centralizado de Compras — segunda peça, complementar à migration anterior
-- (20260828120000_create_inventory_purchases_table.sql).
--
...
--   1) ALTER public.filament_spools: acrescenta initial_gross_weight_grams e
--      purchase_id — migration COMPLEMENTAR sobre uma tabela já aplicada
--      remotamente (20260827103000), nunca uma edição daquela migration já
--      aplicada. Ambas as colunas são nullable (rolos existentes, criados
--      manualmente antes deste incremento, continuam válidos com os dois
--      campos em null — nenhum backfill inventado).
--   2) public.register_inventory_purchase — única função que grava em
--      inventory_purchases, e (para FILAMENT) em filament_types/
--      filament_spools/filament_movements, e (para ACCESSORY/PACKAGING) em
--      stock_movements/accessories.current_stock/packaging.current_stock —
--      sempre reaproveitando as RPCs já existentes e já validadas
--      manualmente (create_filament_type, register_filament_movement,
--      register_stock_movement), nunca duplicando a lógica delas nem
--      escrevendo saldo diretamente. Um único corpo de função = uma única
--      transação Postgres: qualquer exceção não capturada desfaz TUDO que
--      esta chamada já tinha feito até aquele ponto (nenhum rolo órfão,
--      nenhuma movimentação sem compra, nenhuma compra sem efeito completo)
--      — atomicidade por construção, não por controle manual de transação.
--
-- DECISÕES DE ARQUITETURA:
--   - FILAMENT: find-or-create de filament_type pela combinação normalizada
--     material+fabricante+acabamento(line)+cor (case-insensitive, trim) —
--     nunca cria duplicata só por diferença de maiúsculas/espaços. Uma
--     correspondência ativa é reaproveitada; uma correspondência já
--     INATIVA nunca é reativada nem usada silenciosamente — a função
--     levanta FILAMENT_TYPE_INACTIVE_MATCH: para o usuário decidir
--     (reativar o tipo ou ajustar o cadastro) antes de comprar. color_code
--     nunca é exigido nem preenchido a partir de uma compra (decisão já
--     tomada no requisito 2 do pedido, "remover código informado
--     manualmente da interface") — create_filament_type é chamado sempre
--     com p_color_code = null.
--   - Cada rolo comprado grava initial_gross_weight_grams (o peso bruto
--     medido informado na compra) e empty_spool_weight_grams = tara
--     ESTIMADA (peso bruto inicial - peso líquido nominal) — nunca inventa
--     uma tara "de fábrica"; a estimativa pode ser corrigida depois pelas
--     funções de edição/pesagem já existentes (update_filament_spool/
--     register_filament_weighing), sem nenhuma mudança nelas.
--   - O saldo inicial de cada rolo é estabelecido por UMA chamada a
--     register_filament_movement(..., 'PURCHASE', p_nominal_weight_grams,
--     ...) por rolo — nunca uma escrita direta em
--     filament_spools.current_net_weight_grams. reference_type='PURCHASE'/
--     reference_id=<inventory_purchases.id> deixa o vínculo rastreável no
--     histórico de movimentação de cada rolo, sem precisar de uma coluna
--     nova em filament_movements (o par reference_type/reference_id já
--     existia, reservado, desde 20260827110000).
--   - ACCESSORY/PACKAGING: só reaproveita register_stock_movement (já
--     valida item ativo, já atualiza current_stock na mesma transação) —
--     nenhuma lógica nova de saldo é escrita aqui.
--   - Idempotência verificada ANTES de qualquer trabalho pesado (find-or-
--     create de tipo, criação de rolos): uma repetição da MESMA
--     idempotency_key com o MESMO payload devolve a compra já registrada
--     sem refazer nada; payload diferente é rejeitado com
--     IDEMPOTENCY_KEY_CONFLICT:. O caso residual de concorrência real (duas
--     chamadas simultâneas com a mesma chave, nenhuma ainda vendo a outra)
--     é coberto pelo índice único parcial em inventory_purchases
--     (ux_inventory_purchases_idempotency_key, migration anterior): a
--     chamada perdedora recebe um unique_violation (23505) — sem
--     tratamento adicional aqui (diferente de register_stock_movement/
--     register_filament_movement, que capturam esse caso com uma mensagem
--     estável) porque, ao contrário daquelas duas funções, esta gravação
--     nunca é a ÚNICA operação de escrita da chamada — se a chamada
--     perdedora falhar aqui, toda a transação (incluindo qualquer
--     filament_type/rolo que ela possa ter criado até este ponto) desfaz
--     sozinha; o cliente pode simplesmente repetir a mesma requisição, que
--     encontrará a compra já gravada pela vencedora na tentativa seguinte.

alter table public.filament_spools
  add column initial_gross_weight_grams numeric(10, 2)
...
create index idx_filament_spools_purchase_id
  on public.filament_spools (purchase_id)
...
create or replace function public.register_inventory_purchase(
  ...
  -- FILAMENT: dados para localizar/criar o filament_type + criar os rolos.
...
begin
  ...
  -- da migration. p_item_id só é comparado para ACCESSORY/PACKAGING (para
  -- FILAMENT o item ainda não foi resolvido neste ponto); category+
  -- quantity+item_value+freight_value já são um payload suficientemente
  -- específico para o caso real de retry (duplo clique/timeout), mesmo
  -- critério pragmático de "occurred_at fica de fora" já usado em
  -- register_stock_movement/register_filament_movement.
  ...
  -- Find-or-create normalizado (case-insensitive, trim) — nunca cria
  -- duplicata só por diferença de maiúsculas/espaços. order by created_at +
  -- limit 1: se algum dia existir mais de uma correspondência normalizada
  -- (duplicata legada, criada manualmente antes deste incremento), a mais
  -- antiga é reaproveitada de forma determinística, sem erro de "múltiplas
  -- linhas" — mesclar duplicatas legadas fica fora de escopo aqui.
  ...
  if found then
    ...
    -- color_code sempre null: uma compra nunca exige nem inventa um código
    -- de cor (requisito 2 do pedido desta rodada).
